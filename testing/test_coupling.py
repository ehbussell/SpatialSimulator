...
class CouplingRatesTests(unittest.TestCase):
    """Test coupling gives correct infection rates"""

    @classmethod
    def setUp(cls):
        # Setup and run simulation data, single hosts on lattice with exponential kernel

        cls._data_stub = os.path.join("testing", "coupling_rates_sim_output")
        cls._beta_val = 10e3
        cls._scale_val = 1
        size = (25, 25)
        kernel_size = (25, 25)
        ...
        host_file = os.path.join("testing", "coupling_rates_host_test_case.txt")
        host_raster.to_file(host_file)

        # Create initial conditions files
        init_stub = os.path.join("testing", "coupling_rates_init_test_case")
    ...
    def test_coupling_rates(self):
        ...
        ax.set_xlabel("Number of Potential Infection Events")
    ...
    def tearDownClass(cls):
        os.remove(os.path.join("testing", "coupling_rates_config.ini"))
        os.remove(os.path.join("testing", "coupling_rates_host_test_case.txt"))
        for file in glob.glob(os.path.join("testing", "coupling_rates_init_test_case_*")):
            os.remove(file)
